:- dynamic animal/2.

animal(mosca, [huevos, existe, negro, tamano(pequeno), tipo(insecto), habitat(tierra)]).
animal(dodo, [huevos, comer, tamano(grande), tipo(ave), habitat(tierra)]).
animal(unicornio, [cuerno, negro, tamano(grande), tipo(mamifero), habitat(tierra)]).
animal(hamster, [existe, domesticado, negro, olfato, tamano(pequeno), tipo(mamifero), habitat(tierra)]).
animal(ballena, [existe, comer, tamano(grande), tipo(mamifero), habitat(agua)]).
animal(rana, [huevos, existe, tamano(pequeno), tipo(anfibio), habitat(agua)]).
animal(serpiente, [huevos, existe, olfato, tamano(pequeno), tipo(reptil), habitat(tierra)]).
animal(paloma, [huevos, existe, domesticado, negro, comer, tamano(pequeno), tipo(ave), habitat(tierra)]).
animal(loro, [huevos, existe, tamano(pequeno), tipo(ave), habitat(tierra)]).
animal(ciervo, [cuerno, existe, tamano(grande), tipo(mamifero), habitat(tierra)]).
animal(gallina, [huevos, existe, domesticado, comer, tamano(pequeno), tipo(ave), habitat(tierra)]).
animal(gato, [existe, domesticado, negro, olfato, tamano(pequeno), tipo(mamifero), habitat(tierra)]).
animal(perro, [existe, domesticado, negro, olfato, tamano(pequeno), tipo(mamifero), habitat(tierra)]).
animal(lobo, [existe, negro, olfato, tamano(grande), tipo(mamifero), habitat(tierra)]).
animal(gorila, [habitat(tierra), olfato, tamano(grande), negro, tipo(mamifero), existe]).

